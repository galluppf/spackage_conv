...
connE = p.Projection(spike_sourceE, ifcell, p.OneToOneConnector(weights=1, delays=2), target='excitatory')
connI = p.Projection(spike_sourceI, ifcell, p.OneToOneConnector(weights=-1, delays=4), target='inhibitory')
...
ifcell.print_v('results/IF_curr_exp_%s.v' % simulator_name)
