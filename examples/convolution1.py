...
convmodule = p.Population(16384, p.convolution, cell_params, label='convolution_module')
...
connE = p.Projection(spike_sourceE, convmodule, p.ConvolutionConnector(weights=range(16), 
    displacement_x = 1, displacement_y = 1,
    kernel_size_x = 4, kernel_size_y = 4)
    )

connI = p.Projection(spike_sourceI, convmodule, p.ConvolutionConnector(weights=range(-9,0,1), 
    displacement_x = 5, displacement_y = 5,
