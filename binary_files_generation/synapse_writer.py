"""
Synapse writer. Computes the synaptic data structures to be loaded in SDRAM. Can be parallelized in the pacman.cfg file.
...
def compute_sdram_entries(db, c, memory_pointer_base, process_id=0):
    ...
    generate_sdram_connectivity = db.get_image_map(x=c['x'], 
                                                    y=c['y'], 
                                                    p=c['p']
                                                    )[0]['generate_sdram_connectivity']
        
    if  generate_sdram_connectivity == False:
        if PARALLEL: print '[ synapse writer ] : [ Process %d ] : generate_sdram_connectivity False for \
core' % process_id, c['x'], c['y'], c['p']
        else:    print '[ synapse writer ] : generate_sdram_connectivity False for \
core', c['x'], c['y'], c['p']
...
#            if DEBUG:	print part_numpy_synaptic_matrix           
        ...
        # TODO order? implicit indexing for NEF
