...
Each data file has the following structure::
 ...
 - uint reserved2 (NULL)    --> app_data_t.virtual_core_id
 - uint reserved3 (NULL)
 --------------------------
...
def compute_neural_structures(db, processor_id):
    ...
    app_metadata = get_app_metadata(db, part_populations[0])   # only 1 pop per core
    out_file_content += struct.pack("<" + ("I" * len(app_metadata)), *app_metadata)
...
def get_app_metadata(db, part_population):
    """
