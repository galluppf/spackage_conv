...
static uchar  rootChip;               // root chip lead core has spcal functions
static ushort rootAddr;               // lead appl. core has special functions
...
// ----------------------------
static void barrier_setup (uint xchips, uint ychips, uint * core_map);
...
void spin1_set_core_map (uint chips, uint * core_map)
{
  ...
  // only the leader application core does the setup
  if ((ncores > 1) && (leadAp))
  {
    ...
    barrier_setup (bside, bside, core_map);
  }
}
/*
*******/
...
/****f* spin1_api.c/spin1_application_core_map
*
* SUMMARY
*  This function sets the application core map of the simulation
*  The map must be an xchips * ychips rectangle
*
* SYNOPSIS
*  void spin1_application_core_map (uint xc, uint yc, uint core_map[xc][yc])
*
* INPUTS
*  uint xchips: number of chips in the x dimension
*  uint ychips: number of chips in the y dimension
*  uint core_map: bit map of cores involved in the simulation
*
* SOURCE
*/
void spin1_application_core_map (uint xc, uint yc, uint core_map[xc][yc])
{
  ...
  if ((ncores > 1) && (leadAp))
  {
    // TODO: needs extending -- works for rectangular core maps only!
    uint my_x = sv->p2p_addr >> 8;
    uint my_y = sv->p2p_addr & 0xff;
    my_chip = (yc * my_x) + my_y;

    barrier_setup (xc, yc, lcore_map);
  }
}
/*
*******/
...
{
  #if API_DEBUG == TRUE
    // if more than 1 core only the leader appl. core reports router data
    if ((ncores == 1) || (leadAp))
    {
...
*
* SYNOPSIS
*  void barrier_setup (uint xchips, uint xchips, uint * core_map)
*
* SOURCE
*/
void barrier_setup (uint xchips, uint ychips, uint * core_map)
{
  uint i;
  uint my_cores = 0;
  uint my_x = sv->p2p_addr >> 8;
  ...
  // east
  if ((my_x < (xchips - 1)) && (my_y == 0) &&
      ((core_map[my_chip + ychips] & 0xfffffffe) != 0))
  {
    off_route |= (1 << EAST); // add link to east chip
  }

  // north-east
  if ((my_x < (xchips - 1)) && (my_y < (ychips - 1)) &&
      ((core_map[my_chip + ychips + 1] & 0xfffffffe) != 0))
  {
...
void barrier_wait (void)
{
  ...
  if (leadAp)
  {
    ...
    if (rootChip)
    {
      // root node, leader application core
      // send rdygo packet
    ...
    {
      // non-root node, leader application core
  ...
  {
    // all others
    // wait for lock -- from local leader application core
    // timeout if taking too long!
    start = *ms_cnt;          // initial value
...
void rts_init (void)
{
  ...
  // try to become leader application core for this chip
  leadAp = lock_try (LOCK_API_ROOT);

  if (leadAp)
  {
    #if API_DEBUG == TRUE
      io_printf (IO_STD, "\t\t[api_debug] leadAp: %d mon: %d\n",
