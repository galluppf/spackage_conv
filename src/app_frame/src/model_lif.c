...
void timer_callback(uint ticks, uint null)
{
    ...
    for(uint i = 0; i < num_populations; i++)
    {
        ...
        for(uint j = 0; j < population[i].num_neurons; j++, neuron_count++)
        {
            ...
            long long exci_current = curr_psp->exci[ticks % PSP_BUFFER_SIZE];
            long long inhi_current = curr_psp->inhi[ticks % PSP_BUFFER_SIZE];

            // Clear PSP buffers for this timestep
            curr_psp->exci[ticks % PSP_BUFFER_SIZE] = 0;
            curr_psp->inhi[ticks % PSP_BUFFER_SIZE] = 0;
            ...
            curr_psp->exci[(ticks + 1) % PSP_BUFFER_SIZE] += (int) next_exci_current;
            curr_psp->inhi[(ticks + 1) % PSP_BUFFER_SIZE] += (int) next_inhi_current;
