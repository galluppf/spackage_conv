...
void configure_recording_space()
{
	...
	InputSpikesInSDRAM = (SPIKE_LIST_ADDRESS + SPIKE_LIST_SIZE * (app_data.virtual_core_id - 1));
	SDRAMaddress =(unsigned int *)InputSpikesInSDRAM;

    io_printf(IO_STD, "Loading spikes from: %x\n", (SPIKE_LIST_ADDRESS + 
        SPIKE_LIST_SIZE * (app_data.virtual_core_id - 1))); //Used for debug prints
