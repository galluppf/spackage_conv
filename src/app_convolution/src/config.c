...
void configure_app_frame()
{
    ...
    int row_size = sizeof(synaptic_row_t) + sizeof(uint) * app_data.synaptic_row_length;
    io_printf(IO_STD, "app_data.synaptic_row_length: %d, row size: %d bytes\n",app_data.synaptic_row_length, row_size);
...
//        population[i].psp_buffer = population[i - 1].psp_buffer + population[i - 1].num_neurons;
    ...
    // the following section has been moved into each model TODO delete commented section after thourugh tests
